...
# Chunksize is how many words to group together for a sentiment total
# requires python3
...
def get_chunks(filetext, words=50):
    ...
    return [filewords[i:i+words] for i in range(0, len(filewords), words)]

def get_overlap(list1, list2):
    """Determines set-based overlap of 2 lists, returns tuple of counts"""
...
def get_sentiment_counts(chunks, poswords=poswords, negwords=negwords):
    """ Chunks is the size of the pieces to analyse for counts"""
    from collections import Counter
    counts = []
